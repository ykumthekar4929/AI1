...
def plTrue(statement, kb_model):
    if statement.symbol[0]:
        return kb_model[statement.symbol[0]]
    elif statement.connective[0].lower() == connective_and:
        result = True
        for exp in statement.subexpressions:
            result = result and plTrue(exp, kb_model)
        return result
    elif statement.connective[0].lower() == connective_or:
        result = False
        for exp in statement.subexpressions:
            result = result or plTrue(exp, kb_model)
        return result
    elif statement.connective[0].lower() == connective_not:
        return not plTrue(statement.subexpressions[0], kb_model)
    elif statement.connective[0].lower() == connective_if:
        l_sym = statement.subexpressions[0]
        r_sym = statement.subexpressions[1]
        return not (plTrue(l_sym, kb_model) and not plTrue(r_sym, kb_model))
    elif statement.connective[0].lower() == connective_iff:
        l_sym = statement.subexpressions[0]
        r_sym = statement.subexpressions[1]
        return (plTrue(l_sym, kb_model) == plTrue(r_sym, kb_model))
    elif statement.connective[0].lower() == connective_xor:
        result = False
        for exp in statement.subexpressions:
            expTruth = plTrue(exp, kb_model)
            result = (result != expTruth)
        return result
...
def checkEntailment(kb, statement, symbols, kb_model):
    if not symbols:
        if plTrue(kb, kb_model):
            return plTrue(statement, kb_model)
        else:
            return True
    else:
        c_s = symbols.pop(0)
        true_entail = checkEntailment(kb, statement, symbols, update_map(kb_model, c_s, True))
        fals_entail = checkEntailment(kb, statement, symbols, update_map(kb_model, c_s, False))
        return true_entail and fals_entail
...
def get_symbols(expression):
    ...
    if expression.symbol[0]:
        ...
        for subex in expression.subexpressions:
            symbols.extend(get_symbols(subex))
    symbols = list(set(symbols))
    return symbols

def get_negation(statement):
    n_statement = logical_expression()
    n_statement.connective[0] = connective_not
    n_statement.subexpressions.append(statement)
    return n_statement

def get_result(inference, n_inference):
    print "inference %s n_inference %s"%(inference, n_inference)
    if inference and not n_inference:
        result = "definitely true"
    elif not inference and n_inference:
        result = "definitely false"
    elif not inference and not n_inference:
        result = "possibly true, possibly false"
    elif inference and n_inference:
...
def check_true_false(kb, statement):
    symbols = list(set(get_symbols(kb) + get_symbols(statement)))
    kb_model = {}
    c_symbols = list(symbols)

    print "Checking positive"
    inference = checkEntailment(kb, statement, symbols, kb_model,) #entailment for statement

    print "Checking negative"
    n_statement = get_negation(statement)
    n_inference = checkEntailment(kb, n_statement, c_symbols, kb_model) #entailment for negation of statement
    result = get_result(inference, n_inference)
