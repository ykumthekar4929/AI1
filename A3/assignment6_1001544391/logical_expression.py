...
def checkEntailment(kb, statement, symbols, kb_model):
    if not symbols:
        ...
        c_s = symbols.pop(0)
        true_entail = checkEntailment(kb, statement, symbols, extend_model(kb_model, c_s, True))
        fals_entail = checkEntailment(kb, statement, symbols, extend_model(kb_model, c_s, False))
